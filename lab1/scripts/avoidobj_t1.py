...
def callback(data):
    ...
    if minrange>range_max*0.7:
        ...
        flag_rotate=True
        vel_ctl=0
        vel_twist=(10-2)/9*rand+2# the angular velocity is a random variable bweteen 2 and 10 rad/s
...
def control():
    ...
    Pub_Velo=rospy.Publisher('/cmd_vel',Twist, queue_size=10)
    rate=rospy.Rate(10)
    # t0 = rospy.Time.now().to_sec()
    while not rospy.is_shutdown():
        ...
        rospy.Subscriber("/base_scan",LaserScan,callback,queue_size=10)
        rate.sleep()
        # rospy.sleep(/)
        # rospy.spin()
